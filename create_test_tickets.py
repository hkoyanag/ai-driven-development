...
def create_rich_test_tickets():
    """AI仕分けとダッシュボード検証用のバリエーション豊かな15件のテストデータを投入"""
    ...
    # 15件のバリエーション豊かなデータセットの定義
    # start_date（開始日）を期日より前に明示的に設定することでRedmineのエラーを完全回避！
    test_tickets = [
        # --- 【インフラ】セクター ---
        ...
        {"subject": "【インフラ】本番ステージング環境のDBの接続が瞬断する", "desc": "1時間に数回、コネクションプールが枯渇するような挙動があり瞬断が発生しています。ログの調査を要請。", "start": today, "due": today},
        ...
        {"subject": "【インフラ】SSL証明書の更新作業とWebサーバー再起動の計画策定", "desc": "来月末に期限を迎えるSSL/TLS証明書の入れ替え手順書の作成および本番反映のスケジュール調整。", "start": five_days_ago, "due": (today - timedelta(days=2))}, # 💡期限超過（過去日付クリア）
        ...
        {"subject": "【フロント】ダークモード切り替え時のテキスト視認性向上とカラーコード修正", "desc": "ダークモードにした際、特定のリンクテキストが濃い青のままになり背景と同化して読めない不具合。", "start": three_days_ago, "due": (today - timedelta(days=1))}, # 💡期限超過（過去日付クリア）
    ...
    for i, t in enumerate(test_tickets, 1):
        ...
        payload = {
            "issue": {
                ...
                "subject": t["subject"],
                "description": t["desc"],
                "start_date": start_str # 🌟 開始日を明示的にセット
            }
        }
        if due_str:
    ...
    print("\n✨ 15件のバリエーション豊かな検証データの初期セットアップが完了しました！")
